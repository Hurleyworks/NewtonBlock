...
template <class T> class concurrent {
	...
	mutable SharedQueue<concurrent_helper::Callback> _q;
...
public:
	...
	*/
	...
	void shutdown(bool clearQueueFirst = true)
	{
		//LOG(INFO) << "Waiting for concurrent wrapper thread to finish";
		if ( clearQueueFirst )
			_q.clear();
		...
		shutdown_ = true;
		//LOG(INFO) << "Concurrent wrapper thread has completed";
	...
	*/
	...
	unsigned queueSize() { return _q.size(); }
